...
fetch(`${BACKEND_BASE_URL}/meeting/` + meeting_id).then(res => res.json()).then(data => {
    ...
    meeting_date = data['meeting_at'].slice(0, 10)
    meeting_time = data['meeting_at'].slice(11)
    sido1 = data['meeting_city'].slice(0, 4)
    ...
    images.forEach((each_image) => {
        ...
        let temp_html = `
                        ...
                        <a class=image_delete_btn> <img id="delete-image${id}" src="static/image/comment_delete.png" style="width: 30px;" onmouseover="changeDeleteImage(${id})" onmouseout="restoreDeleteImage(${id})" onclick="deleteImage(${id})"> </a>
                        </div>
                        `
